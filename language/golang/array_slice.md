...
1. 练习: 理解append内存占用
    ...
    ```
    - 当 cap 足够时, 不重新复制. 原地扩展. 所以, 当通过make创建 或 grow 后 cap 增长后, 
  通过 append 添加元素, 如果小于cap, 则会原地修改, 则可能更改其他切片值. 
  如上述中, append 生成 c 会更改 b[2] 的值, 更改 c[2] 也会更改 b[2]. 
  而cap不足时, 会重新生成一块内存地址, 此时更改 c[2] 不会更改 b[2]
2. 练习: 理解Go自动扩容与Slice内存占用
    ```Go
    ...
    - 验证以下结论
        ...
        - 本题中, 扩容是由系统自动扩容的. 而第一题, 是手动控制的 cap, 所以第一题更简单些.
