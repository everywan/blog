...
### CAP
对于一个分布式计算系统, 不可能同时满足CAP
- 一致性(Consistence): 所有节点所有时间都有相同的数据副本.
- 可用性(Availability): 每次请求都能在正常响应时间内得到正确相应, 但是不保证返回数据为最新数据.
- 分区容错性(Partition tolerance): 分布式系统在节点或网络分区发生故障时, 仍能对外提供满足一致性和可用性的服务.
